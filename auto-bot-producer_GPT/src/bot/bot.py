...
from src.bot.handlers.sber_text import handle_edit_sber_text_callback
from src.bot.sber_api import format_text_with_sber
...
class Bot:
    ...
    def _setup_handlers(self) -> None:
        ...
        try:
            ...
            # Обработчик для Sber GPT форматирования текста
            self.application.add_handler(
                CallbackQueryHandler(
                    self.handle_sber_text_button,
                    pattern=r"^editsbertext_"
                )
            )
    ...
    async def handle_edit(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if post_context.state != BotState.MODERATE_MENU:
        ...
        # Отправляем клавиатуру редактирования
        await query.message.edit_text(
    ...
    async def handle_edit_media_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Обработчик нажатия на кнопку 'Медиа'."""
        query = update.callback_query
        await query.answer()
        ...
        # Проверяем текущее состояние
        if post_context.state != BotState.EDIT_MENU:
            logger.error(f"Некорректное состояние для редактирования медиа: {post_context.state}")
            await query.message.edit_text("Ошибка: некорректное состояние поста")
            return

        # Меняем состояние на EDIT_MEDIA_WAIT
        old_state = post_context.state
        post_context.state = BotState.EDIT_MEDIA_WAIT
        self.state_manager.set_post_context(post_id, post_context)

        # Отправляем служебное сообщение и сохраняем его ID
        message = await context.bot.send_message(
            chat_id=post_context.chat_id,
            text="Отправьте новые фотографии для поста:"
        )
        post_context.service_messages.append(message.message_id)
        self.state_manager.set_post_context(post_id, post_context)
    ...
    async def handle_add_media_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Обработчик нажатия на кнопку 'Добавить медиа'."""
        query = update.callback_query
        await query.answer()
        ...
        # Проверяем текущее состояние
        if post_context.state != BotState.EDIT_MENU:
            logger.error(f"Некорректное состояние для добавления медиа: {post_context.state}")
            await query.message.edit_text("Ошибка: некорректное состояние поста")
            return

        # Меняем состояние на ADD_MEDIA_WAIT
        old_state = post_context.state
        post_context.state = BotState.ADD_MEDIA_WAIT
        self.state_manager.set_post_context(post_id, post_context)

        # Отправляем служебное сообщение и сохраняем его ID
        message = await context.bot.send_message(
            chat_id=post_context.chat_id,
            text="Отправьте фотографии для добавления к посту:"
        )
        post_context.service_messages.append(message.message_id)
        self.state_manager.set_post_context(post_id, post_context)
    ...
    @check_moderation_block
    async def handle_sber_text_button(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Обработчик нажатия на кнопку 'Текст Sber'."""
        try:
            # Получаем post_id из callback_data
            callback_data = update.callback_query.data
            if not callback_data.startswith("editsbertext_"):
                logger.error(f"Неверный формат callback_data: {callback_data}")
                await update.callback_query.answer("Неверный формат данных")
                return
                
            post_id = callback_data.replace("editsbertext_", "")
            if not post_id:
                logger.error("post_id пустой")
                await update.callback_query.answer("Не удалось определить ID поста")
                return
            ...
            if not current_text:
                await update.callback_query.answer("Нет текста для форматирования")
                return
                
            # Форматируем текст через Sber API
            formatted_text, token_stats = await format_text_with_sber(current_text)
            
            # Обновляем контекст поста
            post_context.original_text = formatted_text
            post_context.was_edited = True
            ...
            if not os.path.exists(post_dir):
                logger.error(f"Папка поста не найдена: {post_dir}")
                await update.callback_query.answer("Ошибка: папка поста не найдена")
                return
            ...
            if not photos:
                logger.error(f"Фотографии не найдены: {post_dir}")
                await update.callback_query.answer("Ошибка: фотографии не найдены")
                return
            
            photo_paths = [os.path.join(post_dir, photo) for photo in photos]
            
            # Отправляем новый пост с фотографиями
            media_group = []
            for i, path in enumerate(photo_paths):
                with open(path, 'rb') as photo:
                    if i == 0:
            ...
            # Сохраняем ID сообщений с медиа
            message_ids = [msg.message_id for msg in messages]
            post_context.original_media = message_ids
            ...
            await update.callback_query.answer("Текст отформатирован")
            
        except Exception as e:
            logger.error(f"Ошибка при форматировании текста: {e}", exc_info=True)
            await update.callback_query.answer("Произошла ошибка при форматировании текста")
