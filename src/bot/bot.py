    ...
    async def process_post(self, post_dir: str, context: ContextTypes.DEFAULT_TYPE) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Network error sending post from {post_dir}: {e}", exc_info=True)
                return False
            except Exception as e:
                logger.error(f"Error sending post from {post_dir}: {e}", exc_info=True)
                return False
            
        except Exception as e:
            logger.error(f"Error processing post from {post_dir}: {e}", exc_info=True)
            return False
    ...
    async def check_posts(self, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Network error sending report: {e}", exc_info=True)
            except Exception as e:
    ...
    async def test_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            if self.check_task is None or self.check_task.done():
                ...
                self.check_task = asyncio.create_task(self._run_periodic_check(context))
                try:
                    await update.message.reply_text("🔄 Запущена периодическая проверка постов")
                except (TimedOut, NetworkError) as e:
                    logger.error(f"Network error sending message: {e}", exc_info=True)
                except Exception as e:
            ...
            try:
                ...
                logger.error(f"Network error sending error message: {e}", exc_info=True)
            except Exception as e:
                logger.error(f"Error sending error message: {e}", exc_info=True)
    
    async def _run_periodic_check(self, context: ContextTypes.DEFAULT_TYPE) -> None:
        """
